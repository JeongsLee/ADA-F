...
class ADAF(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):        
        #self.w = self.add_weight('w', shape=(input_shape[-1],), initializer='random_normal', regularizer = self.kernel_regularizer, trainable=True, dtype=self.DTYPE)
...
class Build_PINN():
    ...
    def init_model_ADAF(self):
        ...
        hidden_split = tf.split(hiddens,20, axis=-1)
        for hidden_unit in hidden_split:
            hidden_concat.append(ADAF(3,3)(hidden_unit))
        hiddens = tf.concat(hidden_concat,axis=-1)
...
class Solver_PINN():
    ...
    def save_results(self, trial):
        ...
        np.savetxt(self.path + 'loss_hist_%s_%s.txt' % (self.cur_pinn.key, trial), np.array(self.loss_history), delimiter=',')
        np.savetxt(self.path + 'acc_hist_%s_%s.txt' % (self.cur_pinn.key, trial), np.array(self.accuracy_history), delimiter=',')
